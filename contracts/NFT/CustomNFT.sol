...
contract CustomNft is ERC721, Ownable, ERC721URIStorage, ERC721Burnable {
    ...
    string public myTokenURI =
        "ipfs://QmSb9KiZKkST5p4cm1bSNJKbD2EtEvv9NmArULNqjoxwpm/";
    ...
    function safeMint(
        address to,
        uint _amtOfTokensToMint
    ) public payable checkAmount(_amtOfTokensToMint) buffer(to) {
        ...
        for (uint i = 0; i < _amtOfTokensToMint; i++) {
            ...
            if (random() == 5) {
                finalURI = string(
                    abi.encodePacked(rareEndpoint, finalURI, baseExtension)
                );
            } else {
                finalURI = string(
                    abi.encodePacked(commonEndpoint, finalURI, baseExtension)
                );
            }
